...
class payment(base):
    ...
    def update_pedido(self,pedido, medio_pago, idpedidoestado, fecha_pago = ''):
        ...
        update_pedido     = {'id' : pedido[0], 'idpedidoestado' : idpedidoestado, 'idmediopago' : medio_pago[0]}
        if '' != fecha_pago:
            update_pedido['fecha_pago'] = fecha_pago
        
        pedido_model.update(update_pedido)
        for direccion in lista_direcciones:
            update_pedido = {'id' : direccion[0], 'idpedidoestado' : 9} # estado de direccion: pago pendiente
            if idpedidoestado==4:
    ...
    def medio(self,var = []):
        ...
        if not isinstance(pedido,tuple):
            if 2 == medio_pago[0]: #  WEBPAY
                try:
                    ...
                    amount      = pedido['total'] # Identificador que será retornado en el callback de resultado:
                    sessionId = pedido['cookie_pedido'] # Identificador único de orden de compra:
                    buyOrder   = pedido['cookie_pedido']
                    returnUrl  = functions.generar_url([self.url[0], 'process' . medio_pago[0], pedido['cookie_pedido']])
                    finalUrl   = functions.generar_url([self.url[0], 'pago' . medio_pago[0], pedido['cookie_pedido']])
        ...
        if not isinstance(pedido,tuple):
            ...
            data['action']= action
            data['form']= form
            data['url_back']= functions.generar_url([seo_cuenta['url'], 'pedido', pedido['cookie_pedido']])
            data['url_next']= functions.generar_url([self.url[0], 'pago' . medio_pago[0], pedido['cookie_pedido']])
