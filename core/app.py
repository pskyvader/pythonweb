    ...
    def init(self, environ):
        ...
        if file_cache != '':
            ...
            if my_file.is_file():
                ...
                if str(type(response))=="<class 'coroutine'>":
                    response={'body':''}
            else:
                response = {'error': 404}
            ...
            if len(url) > 0:
                if url[0] == 'manifest.js':
                    ...
                    if url[1] == 'manifest.js':
                        ...
                        url[1] = 'sw'
                    elif url[1] == 'log.json' or url[1] == 'icon.txt' or url[1] == 'sitemap.xml':
                        url_tmp = [url[0]]
                        del url[0]
        ...
        try:
            for key in p.keys():
                if isinstance(p[key], FieldStorage) and p[key].file:
                    ...
                    if not 'files' in post:
                        post['files']={}
                    if not key in post['files']:
                        post['files'][key]=[]
                    mime = mimetypes.guess_type(name, False)[0]
                    if mime == None:
                        mime = 'text/plain'
                    post['files'][key].append({'name':name,'type':mime, 'tmp_name':tmpfile})
